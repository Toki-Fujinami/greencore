...
*Setup instructions for a gitian build of Cryptogreen using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cryptogreen
...
- [Building Cryptogreen](#building-cryptogreen)
...
Clone the git repositories for cryptogreen and gitian and then checkout the cryptogreen version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cryptogreen repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Cryptogreen
----------------

To build Cryptogreen (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cryptogreen repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cryptogreen/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Cryptogreen
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Cryptogreen.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cryptogreen=${COMMIT} --url cryptogreen=${URL} ../Cryptogreen/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cryptogreen=${COMMIT} --url cryptogreen=${URL} ../Cryptogreen/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cryptogreen=${COMMIT} --url cryptogreen=${URL} ../Cryptogreen/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cryptogreen-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cryptogreen-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cryptogreen-build.assert
```
