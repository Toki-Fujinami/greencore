...
#if defined(NDEBUG)
#error "Cryptogreen cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCryptogreen spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCryptogreen spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCryptogreen spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 21000000 * COIN;  //premine
    } else if(nHeight >=1 && nHeight <= 20000) { //PoW phase
		nSubsidy = 1 * COIN;
    } else if(nHeight >20000 && nHeight <= 2100000) { //PoS phase
		nSubsidy = 500 * COIN;
    } else if(nHeight >2100000 && nHeight <= 4200000) { //PoS phase
		nSubsidy = 250 * COIN;
    } else if(nHeight >4200000 && nHeight <= 6300000) { //PoS phase
		nSubsidy = 125 * COIN;
    } else if(nHeight >6300000 && nHeight <= 8400000) { //PoS phase
		nSubsidy = 62.5 * COIN; 
    } else if(nHeight >10500000 && nHeight <= 12600000) {
		nSubsidy = 31.25 * COIN;
    } else if(nHeight > 12600000 && nHeight <= 14700000) { 
		nSubsidy = 15.62 * COIN;
    } else if(nHeight > 14700000 && nHeight <= 16800000) { 
		nSubsidy = 7.81 * COIN;
    } else if(nHeight > 16800000 && nHeight <= 18900000) { 
		nSubsidy = 3.9 * COIN;
    } else if(nHeight > 18900000 && nHeight <= 21000000) { 
		nSubsidy = 1.95 * COIN;
    } else if(nHeight > 21000000 && nHeight <= 23100000) { 
		nSubsidy = 0.97 * COIN;
    } else if(nHeight > 23100000 && nHeight <= 25200000) { 
		nSubsidy = 0.48 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Cryptogreen
...
{
    RenameThread("cryptogreen-scriptch");
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCryptogreen with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCryptogreenSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCryptogreen spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCryptogreen serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCryptogreen mints and %d zCryptogreen spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Cryptogreen: We use certain sporks during IBD, so check to see if they are
