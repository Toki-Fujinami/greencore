...
// Copyright (c) 2018 The Cryptogreen developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/cryptogreen-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("cryptogreen-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cryptogreen.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cryptogreend.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, cryptogreen, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Cryptogreen specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzcryptogreen=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCryptogreen minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizecryptogreenamount=<n>     " + strprintf(_("Keep N ART anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Cryptogreen Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
{
    RenameThread("cryptogreen-loadblk");
...
/** Sanity checks
 *  Ensure that Cryptogreen is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Cryptogreen Core is shutting down."));
...
#endif
    // Make sure only a single Cryptogreen process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cryptogreen Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Cryptogreen version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Cryptogreen: load previous sessions sporks if we have them.
                ...
                // Cryptogreen: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Cryptogreen Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Cryptogreen Core to complete") << "\n";
        ...
        bool fEnableZCryptogreenBackups = GetBoolArg("-backupzcryptogreen", true);
        pwalletMain->setZCryptogreenAutoBackups(fEnableZCryptogreenBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeCryptogreenAmount = GetArg("-anonymizecryptogreenamount", 0);
//    if (nAnonymizeCryptogreenAmount > 999999) nAnonymizeCryptogreenAmount = 999999;
//    if (nAnonymizeCryptogreenAmount < 2) nAnonymizeCryptogreenAmount = 2;
    ...
    LogPrintf("Anonymize Cryptogreen Amount %d\n", nAnonymizeCryptogreenAmount);
