...
#include "zcryptogreencontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGREEN ought to be enough for anybody." - Bill Gates, 2017
    ui->zGREENpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGREENSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGREEN->setEnabled(false);
        ui->pushButtonMintzGREEN->setToolTip(tr("zGREEN is currently disabled due to maintenance."));

        ui->pushButtonSpendzGREEN->setEnabled(false);
        ui->pushButtonSpendzGREEN->setToolTip(tr("zGREEN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGREENpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGREEN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGREEN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGREEN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGREEN in ") +
...
void PrivacyDialog::on_pushButtonSpendzGREEN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGREEN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGREEN
        sendzGREEN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGREEN
    sendzGREEN();
}

void PrivacyDialog::on_pushButtonZCryptogreenControl_clicked()
{
    ZCryptogreenControlDialog* zCryptogreenControl = new ZCryptogreenControlDialog(this);
    zCryptogreenControl->setModel(walletModel);
    zCryptogreenControl->exec();
}

void PrivacyDialog::setZCryptogreenControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCryptogreenSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGREEN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cryptogreen Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGREENpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGREEN is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGREEN</b>";
    ...
    // use mints from zCryptogreen selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCryptogreenControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCryptogreenControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGREEN transaction
    ...
    // Clear zcryptogreen selector in case it was used
    ZCryptogreenControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCryptogreen Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Cryptogreen, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCryptogreen Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGREEN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGREEN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGREEN "));
    ui->labelzGREENAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGREENSyncStatus->setVisible(fShow);
}
