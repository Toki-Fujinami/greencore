...
#include "ui_zcryptogreencontroldialog.h"
...
std::list<std::string> ZCryptogreenControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCryptogreenControlDialog::listMints;

ZCryptogreenControlDialog::ZCryptogreenControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCryptogreenControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCryptogreenControlDialog::~ZCryptogreenControlDialog()
{
    delete ui;
}

void ZCryptogreenControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCryptogreenControlDialog::updateList()
{
...
void ZCryptogreenControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCryptogreenControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCryptogreen_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCryptogreenControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCryptogreenControlDialog::GetSelectedMints()
{
...
void ZCryptogreenControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
