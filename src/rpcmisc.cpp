...
// Copyright (c) 2017-2018 The Cryptogreen developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cryptogreen balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cryptogreen/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cryptogreen/kb\n"
    ...
    Object zcryptogreenObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcryptogreenObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcryptogreenObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zARTsupply", zcryptogreenObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cryptogreenaddress\"\n"
            "\nReturn information about the given cryptogreen address.\n"
            "\nArguments:\n"
            "1. \"cryptogreenaddress\"     (string, required) The cryptogreen address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cryptogreenaddress\", (string) The cryptogreen address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Cryptogreen address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cryptogreen addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cryptogreen address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cryptogreenaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cryptogreenaddress\"  (string, required) The cryptogreen address to use for the signature.\n"
